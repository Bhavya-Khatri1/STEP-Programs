...
public class UniversitySystem {
    public static void main(String[] args) {
        ...
        Person p = alice; // upcasting — safe
        p.introduce();
        p.getContactInfo();

        // p.attendLecture(); // <-- compile-time error: Person doesn't have attendLecture()

        System.out.println("Accessing name via Person reference: " + p.name);

        // Downcast to call subclass-specific method:
        if (p instanceof Student) {
            ((Student) p).attendLecture();
        }
    }
}